...
- whether the app-reported byte accounting matches predictable UTF-8 JSON size;
...
- matches only normal Torn top-level pages for this phase;
...
- keeps quota testing behind a separate explicit button;
...
5. Confirm the reported quota is expected to be the new probe's default 10 MiB.

If native storage is unavailable, stop.
...
- approximately 64 KiB;
- approximately 256 KiB;
- approximately 1 MiB.
...
- `PDA_storage.set()` duration;
- observed `usage()` delta after write;
- whether observed byte accounting equals expected byte accounting;
- `PDA_storage.get()` duration;
- exact serialized round-trip equality;
- expected/actual compact hashes;
- verification/stringification duration;
- optional `loadAll()` duration for selected largest cases;
- delete duration;
...
- a measured storage bridge operation exceeds the probe's conservative slow-operation threshold.

An abort is evidence, not something to work around during the same run.

## Phase Q2 — Batch profile

After scaling cases pass, the same run compares an approximately 1 MiB total workload split across multiple values using:

- `setMany()`;
- `getMany()`;
- cleanup by key.

The batch report records total serialized bytes, write/read duration, exact round-trip verification for every entry, usage delta, and cleanup return-to-baseline.
...
The v0.1.0 quota test is designed around a fresh default 10 MiB qualification namespace.

### Procedure performed by the probe

1. Clean stale qualification keys.
2. Record baseline usage and quota.
3. Write a small sentinel/control record.
4. Fill the disposable namespace to roughly 80% of its reported quota using bounded synthetic chunks.
5. Record pre-failure usage.
6. Attempt one additional value whose calculated encoded size must exceed the remaining namespace quota by a modest margin.
7. Expect JavaScript error code `QuotaExceeded`.
8. Verify:
   - the rejected key does not exist;
   - namespace usage did not increase;
   - the sentinel is unchanged;
   - previously successful fill records remain readable.
9. Attempt a `setMany()` where each individual member is reasonable but the combined batch must exceed remaining quota.
10. Expect `QuotaExceeded` and verify **neither** batch key was inserted.
11. Verify sentinel again.
12. Delete all qualification keys.
13. Confirm namespace usage returns to the initial clean baseline.

### Guardrails

The quota action refuses or stops rather than scaling indefinitely if the namespace quota is unexpectedly large. v0.1.0 is intended to qualify the normal 10 MiB default, not a 50 MiB override.
