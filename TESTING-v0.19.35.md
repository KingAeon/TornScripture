# TESTING-v0.19.35.md — Manual Verification Plan
...
1. Manually record a sale for the same item/quantity/proceeds using **Recover missed sale** (manual path). If available, also repeat with a same-trade manual record created through the completed-trade path so the stored counterparty, direction, and totals are populated.
...
- Prior stable version: IMM `0.19.34` at `da7d6a6aa4fad152158be9ccb5f3403a6ee96960`
- Reinstall: replace the userscript with the prior version from the stable commit.
- Additive fields only (`soldAt`, API identity fields, and `tradeDirection` on newly recorded sales); no storage key rename or destructive migration.
