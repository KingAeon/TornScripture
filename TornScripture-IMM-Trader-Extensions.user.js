...
(() => {
  ...
  const A = Object.freeze({
    v: '0.1.3', bridge: 'TSIMM_PRICE_BRIDGE:', traders: 'tornscripture-imm-traders-v1',
  ...
  function reportTrader(ts) {
    ...
    const byHeader = headerKey ? ts.find((x) => x.n === headerKey) : null;
    if (byHeader) {
      activeTrader = byHeader.id;
      return byHeader;
    }

    const visibleNames = [...overlay.querySelectorAll('.td-row-title strong')]
      .slice(0, 12)
      .map((node) => key(node.textContent))
      .filter(Boolean);
    if (visibleNames.length) {
      let winner = null;
      let best = 0;
      for (const trader of ts) {
        const names = new Set(trader.items.map((item) => item.n));
        const score = visibleNames.reduce((sum, name) => sum + (names.has(name) ? 1 : 0), 0);
        if (score > best) {
          best = score;
          winner = trader;
        }
  ...
  function decorateDeals() {
    ...
    const trader = reportTrader(ts);
    if (!trader) return;
    const store = trackedStore();

    for (const row of overlay.querySelectorAll('.td-row')) {
      try {
        ...
        let button = row.querySelector('[data-track-toggle]');
        const tracked = store.entries.some((entry) => (
        ...
        if (!button) {
          ...
          button.dataset.trackToggle = '1';
          button.setAttribute('role', 'button');
          button.setAttribute('tabindex', '0');
          title.appendChild(button);
        }

        button.className = `tsimm-track-toggle${tracked ? ' on' : ''}`;
        button.textContent = tracked ? '✓ TRACKED' : '+ TRACK';
        button.setAttribute('aria-label', `${tracked ? 'Untrack' : 'Track'} ${item.name} for ${trader.name}`);
  ...
  function tornBoot() { if (importTX()) return; notice(); const start = () => { if (!document.body) return setTimeout(start, 60); injectStyle(); document.addEventListener('click', (e) => { const d = e.target.closest?.('[data-tsimm-deals-open]'); if (d?.dataset?.tsimmTraderId) { activeTrader = d.dataset.tsimmTraderId; setTimeout(scheduleTorn, 0); } const r = e.target.closest?.('[data-tx-recapture]'); if (r) { e.preventDefault(); e.stopPropagation(); const t = normTraders().find((x) => x.id === clean(r.dataset.trader)); if (t) openRecapture(t); return; } const b = e.target.closest?.('[data-track-toggle]'); if (b) { e.preventDefault(); e.stopImmediatePropagation(); const t = normTraders().find((x) => x.id === clean(b.dataset.trader)), it = t?.items.find((x) => (Number(b.dataset.item) > 0 && x.id === Number(b.dataset.item)) || x.n === key(b.dataset.name)); if (t && it) toggleTrack(t, it); } }, true); new MutationObserver(() => { if (!ownMutation) scheduleTorn(); }).observe(document.body, { childList: true, subtree: true }); setInterval(scheduleTorn, 1500); scheduleTorn(); }; start(); }
